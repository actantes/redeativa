...
public class FBServ extends Service {

	 private WakeLock mWakeLock;
	SocialAuthAdapter	adapterf;
	 OItem eve = new OItem();
	  ...
	  public int onStartCommand(Intent intent, int flags, int startId) {
		  ...
		  ConnectivityManager cm =
			...
			if(isConnected&&Logado){
				adapterf = new SocialAuthAdapter(new ResponseLs());
	 			adapterf.addProvider(Provider.FACEBOOK, R.drawable.facebook);

		 		try {
					adapterf.addConfig(Provider.FACEBOOK, "AppDevID", "AppDevSecret", null);
					
		 		
		 		} catch (Exception e) {
		 			 ReAlarm();
		 		}
		 		adapterf.authorize(getApplicationContext(), Provider.FACEBOOK);
		...
		private final class ResponseLs implements DialogListener {
			...
			public void onComplete(Bundle values) {
				...
				Map<String, Object> params = new HashMap<String, Object>();
	          	...
	          	params.put("post", id);
	          	AQuery aq = new AQuery(FBServ.this);
	                	...
	                	if(val != null){
	                            ...
	                            JsonObject obj = parser.parse(val).getAsJsonObject();
	                         	 ...
	                         	 Aval.clear();
	                         	  ...
	                         	  adapterf.updateStatus(eve.getbody(), new MessageLs(),false);
	                           //                          } 
	                        }else{ // val null
	                        	  ReAlarm();
	                        	stopSelf();
			...
			@Override
			public void onError(SocialAuthError error) { // erro de autenticação
		...
		private final class MessageLs implements SocialAuthListener<Integer> {
			...
			public void onExecute(String provider, Integer t) {
				...
				if (status.intValue() == 200 || status.intValue() == 201 || status.intValue() == 204){
					...
					CriaNotSev("RedeAtiva","Compartilhou no FB "+Integer.toString(id),id);
					...
					CriaNotSev("RedeAtiva","Não compartilhou no FB "+Integer.toString(id),id);
					  ReAlarm();
				}
			}
	    ...
	    public void CriaNotSev (String Titulo,String Texto,int nID){
		  ...
		  public void ReAlarm(){
		     	 ...
		     	 if(cur_cal.get(Calendar.HOUR_OF_DAY)+1<24){
		     	 		...
		     	 		Intent ita = new Intent(FBServ.this, FBServ.class);
		     	 		ita.putExtra("PID",Integer.toString(id));
		 				PendingIntent pia = PendingIntent.getService(FBServ.this,
		 						id, ita,  PendingIntent.FLAG_UPDATE_CURRENT);
		 				am.cancel(pia); 
